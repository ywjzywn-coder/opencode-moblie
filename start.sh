...
#
# 启动 3 个服务:
#   1. Relay    - WebSocket 中继 + PWA 静态文件服务 (端口 4097)
#   2. opencode  - opencode serve HTTP API (端口 4096)
#   3. Daemon    - 连接 relay + opencode 的守护进程
#
# PWA 由 relay 内置 HTTP 服务器直接提供（生产构建），无需 Vite 开发服务器。
#
# 用法: ./start.sh
# 停止: Ctrl+C
...
# 0. 构建 PWA 生产版本
echo "[0/3] 构建 PWA..."
pnpm --filter @opencode-remote/pwa build > /tmp/oc-remote-pwa-build.log 2>&1
echo "      PWA 构建完成"

# 1. 启动中继服务器 (同时提供 PWA 静态文件)
echo "[1/3] 启动中继服务器 (端口 $RELAY_PORT)..."
node packages/relay/dist/cli.js > /tmp/oc-remote-relay.log 2>&1 &
RELAY_PID=$!
sleep 1.5

# 2. 启动 opencode serve (本地 HTTP API)
echo "[2/3] 启动 opencode (端口 $OPENCODE_PORT)..."
...
cleanup() {
  echo ""
  echo "正在停止..."
  kill $RELAY_PID $OC_PID $DAEMON_PID 2>/dev/null || true
  pkill -f "relay/dist/cli" 2>/dev/null || true
  pkill -f "opencode serve --port $OPENCODE_PORT" 2>/dev/null || true
  pkill -f "plugin/dist/daemon" 2>/dev/null || true
  echo "已停止"
